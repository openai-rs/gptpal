...
pub(crate) fn write_file(file_path: &str, data: String) {
	...
	file.write(data.as_bytes()).expect("Failed to write conversations to file");
}
...
pub fn update_api(config: Config) {
	...
	if let Some(api_key) = config.api_key {
		...
		auth = Auth::from_env().unwrap_or(Auth { api_key: "".to_string(), organization: None });
	}
	...
	let mut openai = OpenAI::new(auth, &api_url);
